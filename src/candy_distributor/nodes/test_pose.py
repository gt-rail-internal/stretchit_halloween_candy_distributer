...
class SinglePointCommand(hm.HelloNode):
    def __init__(self):
    ...
    def main(self):
        hm.HelloNode.main(self, 'singlepoint_command', 'singlepoint_command', wait_for_first_pointcloud=False)
        ...
        poses = {
            ...
            "pre_grasp_2": [0.20268437786715443, 0.7513784372841598, 0.3, 0.1674595693441825, -1.8338199879850292, 0.0],
        ...
        while not rospy.is_shutdown():
            choice = input("Enter the name of the pose you'd like to test or 'q' to quit: ")
            if choice in poses:
                self.issue_singlepoint_command(poses[choice])
                time.sleep(2)
            elif choice == 'q':
                break
            else:
                print("Invalid choice. Please choose from the available poses or enter 'q' to quit.")

if __name__ == '__main__':
    try:
        node = SinglePointCommand()
        node.main()
